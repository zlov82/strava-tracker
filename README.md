...
| `GET` | `/api/stats/{year}/{month}` | — | Статистика по видам спорта за месяц: кол-во тренировок, километраж, время; для велосипеда — суммарный набор высоты |
...
Это предотвращает экспоненциальный рост начальной загрузки страницы по мере накопления кэша: `streams_raw` одной активности может занимать 100–500 КБ.
